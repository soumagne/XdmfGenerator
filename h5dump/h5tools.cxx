...
h5tools_print_region_data_blocks(hid_t region_id,
    ...
    for (blkndx = 0; blkndx < nblocks; blkndx++) {
        ...
        for (jndx = 0; ((unsigned int) jndx) < ctx.ndims; jndx++)
        ...
        elmtno = 0;
        for (jndx = 0; ((unsigned int) jndx) < numelem; jndx++, elmtno++, ctx.cur_elmt++) {
            ...
            if (((unsigned int) jndx + 1) < numelem || (region_flags & END_OF_DATA) == 0)
                h5tools_str_append(buffer, "%s", OPT(info->elmt_suf1, ","));
                           ...
                           unsigned int row_dim/* index of row_counter dimension */)
{
    ...
    for (; hyperslab_count > 0; temp_start[row_dim] += temp_stride[row_dim], hyperslab_count--) {
        ...
        if (sm_nelmts > 0) {
            ...
            if((sm_buf = (unsigned char*)HDmalloc((size_t) sm_nelmts * p_type_nbytes)) == NULL)
                        ...
                        hid_t type, hid_t space, void *mem, int indentlevel)
{
    ...
    /* Assume entire data space to be printed */
    for (i = 0; ((unsigned int) i) < ctx.ndims; i++)
        ctx.p_min_idx[i] = 0;

    H5Sget_simple_extent_dims(space, ctx.p_max_idx, NULL);

    for (i = 0, nelmts = 1; ctx.ndims != 0 && (((unsigned int) i) < ctx.ndims); i++)
...
h5tools_print_dataspace(h5tools_str_t *buffer, hid_t space)
{
    ...
    int         ndims = -1;
    H5S_class_t space_type = H5S_NO_CLASS;
    int         i;
...
h5tools_print_enum(h5tools_str_t *buffer, hid_t type)
{
    ...
    if((name = (char**)calloc(nmembs, sizeof(char *))) == NULL)
        H5E_THROW(FAIL, H5E_tools_min_id_g, "Could not allocate buffer for member name");
    if((value = (unsigned char*)calloc(nmembs, MAX(type_size, dst_size))) == NULL)
    ...
    hid_t container, int ndims, hid_t type_id, hssize_t nblocks, hsize_t *ptdata)
{
    ...
    for (blkndx = 0; blkndx < nblocks; blkndx++) {
        ...
        for (jndx = 0; ((unsigned int) jndx) < numelem; jndx++) {
